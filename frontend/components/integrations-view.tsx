...
function StatusBadge({label}:{label:string}) { const positive=label==="Connected — real"; const warning=label==="Configured but unverified"||label==="Synthetic demo"; return <Badge variant="outline" className={positive?"border-teal-200 bg-teal-50 text-teal-800":warning?"border-amber-200 bg-amber-50 text-amber-900":"border-slate-200 bg-slate-50 text-slate-600"}>{positive?<BadgeCheck/>:<CircleAlert/>}{label}</Badge>; }
...
export function IntegrationsView({status, onRefresh}:{status:IntegrationStatus|null;onRefresh:()=>Promise<void>}) {
  ...
  const cards=[
    {key:"gmail",icon:Mail,title:"Gmail",copy:"Read-only attachment discovery. Email bodies are not retained.",state:status?.gmail.status||"Error",detail:status?.gmail.account||"Synthetic adapter remains available for development."},
